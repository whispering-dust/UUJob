...
public class RecommendController {
    ...
    public Result calculateSimilarity() {
        ...
        for (UserSimilarity uSim : similarityList) {
            //System.out.println(uSim.getUserId() + "\t" + uSim.getRefUserId() + "\t" + uSim.getSimilarity());
            // 5.如果用户之间的相似度已经存在于数据库中就修改，不存在就添加
            int id = userSimilarityService.isExistsUserSimilarity(uSim);
            System.out.println(uSim + "的实际id是" + id);
            if (id != -1) { // 修改
                uSim.setId(id);
