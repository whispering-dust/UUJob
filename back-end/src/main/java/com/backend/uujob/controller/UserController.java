...
import com.backend.uujob.vo.JobVO;
import com.backend.uujob.vo.ResponseVO;
import com.backend.uujob.vo.StarVO;
...
public class UserController {
    ...
    public Result getApplicationByUserId(@RequestParam int userId) {
        ...
        for (Application a : applicationList) {
            ...
            if (!Objects.equals(ja.getStatus(), ApplStatusEnum.APPL_STATUS_SUBMIT.getValue())) {  //申请有结果才需要添加批复日期，否则不需要
    ...
    public Result getStarByUserId(@RequestParam int userId) {
        ...
        for (Star s : starList) {  //根据收藏信息收集完整的显示信息
            ...
            if (Objects.equals(starVO.getStarType(), StarTypeEnum.STAR_TYPE_POST.getValue())) {  //根据收藏的类别选择对应的标题
                Post post = postService.getById(starVO.getTargetId());
                starVO.setTitle(post.getTitle());
            }
            if (Objects.equals(starVO.getStarType(), StarTypeEnum.STAR_TYPE_JOB.getValue())) {
